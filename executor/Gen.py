...
def run(id_, batch, device, model, attack, number_of_steps, data_queue, split, split_size, gen=True):
    ...
    if(gen):
        ...
        if(split == -1 or split == 1):
            ...
            image_normal = torch.split(image_normal, int(split_size * 3 / 4))
            image_adversarial = torch.split(image_adversarial, int(split_size/4))
            label_normal = torch.split(label_normal, int(split_size * 3 / 4))
            label_adversarial = torch.split(label_adversarial, int(split_size/4))
